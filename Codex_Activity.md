...
_Shilad Sen (with help from Codex), 2025/11/25_
_Natalia and Iris._

In this activity you will:

- Use Codex as a collaborator on a real spatial dataset.
- Practice being _critical_ of Codex’s suggestions (not just accepting them).
...
- What did Codex assume or gloss over?
  It assumed it was for an assigment, It is giving me soecific steps on how to view the dataset, assuming we are using Python
- Did it make any mistakes or questionable choices?
  Not really. It gave us detail steps to follow so we could explore the data on our own as well.
- How clear or explainable was the code it produced?
  The exploration is not explained in depth, but the code chunks are roken down in steps and some code is commented.

# 4. More directed exploration

- If you are partner coding, _switch partners_.
...
- What did Codex do well?
  It switched coding language as asked. It couldn't show the visuals as required because of we didn't have the "Agent" mode. It also used the theme minimal which I appreciate as it uses less ink, given that these are just exploratory visualizations.
- What would you have done differently if you wrote the code yourself?
  I can't tell what I would have done dfferently. It flipped the axes using coor flip for most of the bar graphs which I would have done, used the theme minimal, commented the code lined for different summaries of mutate, and labeled the graphs.
- Looking at the code Codex wrote, what could be improved (style, clarity, correctness, efficiency)? I personally htink the code is clear, legible and reproducible.
...
- Describe what was easier and harder about doing this with Codex.
  The AI figured how to make a simple stairghtforward map of the data, it showed me how to map the point for different neighborhoods, thought the code could have been more commented.
- Where did Codex help, and where did it get in the way? It helped with the full code to do the basic mapping.
- What would you do differently next time you had to map a new dataset with AI assistance?
  I would maybe tell it how to name the different data frames it was creating to filter necessary data, just so I can ensure consistency in naming.
...
- List at least two candidate research questions that Codex suggested.
  How are on-sale liquor licenses distributed across neighborhoods and wards?
  What proportions of licenses are approved or pending or other statuses, and does this mix differ by neighborhood or ward?
- For each, briefly note what seems interesting and what might be problematic (data, scope, ethics, feasibility).
  I love that both look at differences between neighborhoods, as it could be interesting to see if there is any noticeable agreement or disagreement. For the second one I also love that it looks at proportions of licenses, as this helps account for the different number of licences that would be in different neighborhoods. I think the questions are feasible and interesting enough, and fit the scope o the data.
...
- Now ask Codex to **plan first, then code** for the _same_ research question. For example:
...
- Then, _step by step_, ask Codex to write code for each step:
